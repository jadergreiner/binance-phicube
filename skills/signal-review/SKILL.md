---
name: signal-review
description: Workflow replicado de .claude/commands/signal-review.md para 'Revisão de Sinal Phicube'. Use quando precisar executar esse procedimento no projeto Binance Phicube.
---
...
**Argumento:** módulo ou função a revisar (ex: `signal_engine.py`, `indicators.py`, `evaluate()`)
...
- `tests/test_indicators.py` e `tests/test_signal_engine.py`
...
```bash
pytest tests/test_indicators.py tests/test_signal_engine.py -v
```

### 7. Consultar o agente trader-senior

Se qualquer item dos passos 2–5 falhar ou a mudança for substancial, acione o agente `trader-senior` e aguarde aprovação explícita antes de avançar.
